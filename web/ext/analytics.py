...
class TimingSuffix:
	"""Record the "end time" of each extension callback phase."""
	
	last: bool = True  # This extension goes "last" in the execution stack for each extension callback.
	provides: Tags = {'timing.suffix'}  # Expose this symbol for other extensions to depend upon.
...
class TimingExtension:
	"""Calculate and generate HTTP response headers and logging output related to request processing timing."""
	
	__slots__ = ('header', 'log', 'timing', 'done')
	
	last: bool = True
	needs: Tags = {'timing.prefix', 'timing.suffix'}  # We depend on the timing information generated by these.
	uses: Tags = {'args.validation'}
	provides: Tags = {'analytics'}  # Expose this symbol for other extensions to depend upon.
	...
	def transform(self, context:Context, endpoint:Callable, result:Any) -> Any:
		"""Executed to transform outgoing values, after endpoint invocation, prior to view resolution."""
		
		delta = context.milestone['transform'] - context.milestone['collect-']
	...
	def _done(self, context:Context) -> None:
